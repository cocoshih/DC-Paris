...
function drawCodedImage() {
  textImageCtx.drawImage(
    this,
    0,
    0,
    textImageCanvas.width,
    textImageCanvas.height
  );
}
...
function drawInputImage() {
  ...
  if (message.value != "") {
    ...
    img.src = URL.createObjectURL(
      document.getElementById("codedImageFile").files[0]
    );
    textImageCtx.fillStyle = "white";
    textImageCtx.fillRect(0, 0, textImageCanvas.width, textImageCanvas.height);
  }
...
function hide() {
  var textImagePixels = textImageCtx.getImageData(
    0,
    0,
    textImageCanvas.width,
    textImageCanvas.height
  );
  var inputImagePixels = inputImageCtx.getImageData(
    0,
    0,
    inputImageCanvas.width,
    inputImageCanvas.height
  );
  var outputImagePixels = inputImageCtx.getImageData(
    0,
    0,
  ...
  for (var i = 0; i < outputImagePixels.data.length; i += 4) {
    outputImagePixels.data[i] =
      (inputImagePixels.data[i] & 0xfe) | ((textImagePixels.data[i] >> 7) & 1);
  }
